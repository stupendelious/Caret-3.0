...
public class APITransactionManagerImpl implements APITransactionManager {
  ...
  @Override
  public JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, Long recipientId, long amountNQT, Attachment attachment, long minimumFeeNQT) throws CaretException {
    ...
    if (messageValue != null) {
      boolean messageIsText = Caret.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchainHeight)
    ...
    } else if (attachment instanceof Attachment.ColoredCoinsAssetTransfer && Caret.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchainHeight)) {
    ...
    } else if (attachment == Attachment.ARBITRARY_MESSAGE && ! Caret.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchainHeight)) {
    ...
    if (recipientPublicKey != null && Caret.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchainHeight)) {
    ...
    } catch (CaretException.NotYetEnabledException e) {
