...
public class SqlEscrowStore implements EscrowStore {
  private final CaretKey.LongKeyFactory<Escrow> escrowDbKeyFactory = new DbKey.LongKeyFactory<Escrow>("id") {
      @Override
      public CaretKey newKey(Escrow escrow) {
        return escrow.dbKey;
      }
    };
  ...
  private final DbKey.LinkKeyFactory<Escrow.Decision> decisionDbKeyFactory =
      new DbKey.LinkKeyFactory<Escrow.Decision>("escrow_id", "account_id") {
        @Override
        public CaretKey newKey(Escrow.Decision decision) {
  ...
  protected void saveDecision(DSLContext ctx, Escrow.Decision decision) throws SQLException {
    ...
    decisionRecord.setHeight(Caret.getBlockchain().getHeight());
  ...
  @Override
  public CaretKey.LongKeyFactory<Escrow> getEscrowDbKeyFactory() {
  ...
  public Collection<Escrow> getEscrowTransactionsByParticipant(Long accountId) {
    ...
    CaretIterator<Escrow.Decision> it = decisionTable.getManyBy(ESCROW_DECISION.ACCOUNT_ID.eq(accountId), 0, -1);
    while (it.hasNext()) {
  ...
  protected void saveEscrow(DSLContext ctx, Escrow escrow) throws SQLException {
    ...
    escrowRecord.setHeight(Caret.getBlockchain().getHeight());
  ...
  @Override
  public 	CaretIterator<Escrow.Decision> getDecisions(Long id)
  {
