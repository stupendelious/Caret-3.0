...
public final class Peers {
  ...
  public static void init(TimeService timeService, AccountService accountService, Blockchain blockchain, TransactionProcessor transactionProcessor,
    ...
    if (myAddress != null && myAddress.endsWith(":" + TESTNET_PEER_PORT) && !Caret.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
      throw new RuntimeException("Port " + TESTNET_PEER_PORT + " should only be used for testnet!!!");
    }
    myPeerServerPort = propertyService.getInt(Props.P2P_PORT);
    if (myPeerServerPort == TESTNET_PEER_PORT && !Caret.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
      throw new RuntimeException("Port " + TESTNET_PEER_PORT + " should only be used for testnet!!!");
    }
    useUpnp = propertyService.getBoolean(Props.P2P_UPNP);
    shareMyAddress = propertyService.getBoolean(Props.P2P_SHARE_MY_ADDRESS) && ! Caret.getPropertyService().getBoolean(Props.DEV_OFFLINE);
    ...
    if (myAddress != null && ! myAddress.isEmpty()) {
      try {
        ...
        if (!Caret.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
          if (port >= 0) {
    ...
    json.put("application",  Caret.APPLICATION);
    json.put("version",      Caret.VERSION);
    ...
    if(propertyService.getBoolean(P2P_ENABLE_TX_REBROADCAST)) {
      rebroadcastPeers = Collections
          .unmodifiableSet(new HashSet<>(propertyService.getStringList(Caret.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Props.DEV_P2P_REBROADCAST_TO : Props.P2P_REBROADCAST_TO)));
    } else {
      rebroadcastPeers = Collections.emptySet();
    }

    List<String> wellKnownPeersList = propertyService.getStringList(Caret.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Props.DEV_P2P_BOOTSTRAP_PEERS : Props.P2P_BOOTSTRAP_PEERS);
    ...
    if (wellKnownPeersList.isEmpty() || Caret.getPropertyService().getBoolean(Props.DEV_OFFLINE)) {
    ...
    usePeersDb       = propertyService.getBoolean(Props.P2P_USE_PEERS_DB) && ! Caret.getPropertyService().getBoolean(Props.DEV_OFFLINE);
    ...
    threadPool.runBeforeStart(new Runnable() {
        ...
        public void run() {
          ...
          if (usePeersDb) {
            ...
            loadPeers(Caret.getDbs().getPeerDb().loadPeers());
          }
    ...
    if (! Caret.getPropertyService().getBoolean(Props.DEV_OFFLINE)) {
  ...
  private static class Init {
    ...
    static void init(TimeService timeService, AccountService accountService, Blockchain blockchain, TransactionProcessor transactionProcessor,
      ...
      if (Peers.shareMyAddress) {
            ...
            if (gateway != null) {
              ...
              try {
                ...
                if (gateway.getSpecificPortMappingEntry(port, "TCP", portMapping)) {
                  ...
                  if (gateway.addPortMapping(port, port, localAddress.getHostAddress(), "TCP", "caretcoin")) {
        ...
        port = Caret.getPropertyService().getBoolean(Props.DEV_TESTNET) ? TESTNET_PEER_PORT : Peers.myPeerServerPort;
  ...
  private static final Runnable peerConnectingThread = new Runnable() {
      ...
      try {
        ...
        while ( ! Thread.currentThread().isInterrupted() && ThreadPool.running.get()
          ...
          if (peer != null) {
            ...
            if (!peer.isHigherOrEqualVersionThan(Caret.LEGACY_VER)
    ...
    private void updateSavedPeers() {
      Set<String> oldPeers = new HashSet<>(Caret.getDbs().getPeerDb().loadPeers());
      Set<String> currentPeers = new HashSet<>();
      for (Peer peer : Peers.peers.values()) {
        if (peer.getAnnouncedAddress() != null
        && ! peer.isBlacklisted()
        && ! peer.isWellKnown()
        && peer.isHigherOrEqualVersionThan(Caret.LEGACY_VER)) {
      ...
      try {
        Caret.getStores().beginTransaction();
        Caret.getDbs().getPeerDb().deletePeers(toDelete);
     //   logger.debug("Deleted " + toDelete.size() + " peers from the peers database");
        currentPeers.removeAll(oldPeers);
        Caret.getDbs().getPeerDb().addPeers(currentPeers);
     //   logger.debug("Added " + currentPeers.size() + " peers to the peers database");
        Caret.getStores().commitTransaction();
      } catch (Exception e) {
        Caret.getStores().rollbackTransaction();
        throw e;
      } finally {
        Caret.getStores().endTransaction();
      }
    }
          ...
          try {
            ...
            for (Peer myPeer : Peers.getAllPeers()) {
              if (! myPeer.isBlacklisted() && myPeer.getAnnouncedAddress() != null
                  ...
                  && myPeer.isHigherOrEqualVersionThan(Caret.LEGACY_VER)
                  ) {
  ...
  static PeerImpl addPeer(final String address, final String announcedAddress) {
    ...
    if (Caret.getPropertyService().getBoolean(Props.DEV_TESTNET) && peer.getPort() > 0 && peer.getPort() != TESTNET_PEER_PORT) {
  ...
  private static boolean peerEligibleForSending(Peer peer, boolean sendSameBRSclass) {
    return peer.isHigherOrEqualVersionThan(Caret.LEGACY_VER)
  ...
  public static Peer getAnyPeer(Peer.State state) {
    ...
    if(!connectWellKnownFinished) {
      ...
      if (wellKnownConnected >= connectWellKnownFirst) {
        ...
        logger.info("You can open your Caret Wallet in your favorite browser with: http://127.0.0.1:8125 or http://localhost:8125");
      }
    }
