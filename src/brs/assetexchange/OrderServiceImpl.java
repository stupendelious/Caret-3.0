...
class OrderServiceImpl {
  ...
  public CaretIterator<Ask> getAllAskOrders(int from, int to) {
    return askOrderTable.getAll(from, to);
  }

  public CaretIterator<Bid> getAllBidOrders(int from, int to) {
    return bidOrderTable.getAll(from, to);
  }

  public CaretIterator<Bid> getSortedBidOrders(long assetId, int from, int to) {
    return orderStore.getSortedBids(assetId, from, to);
  }

  public CaretIterator<Ask> getAskOrdersByAccount(long accountId, int from, int to) {
    return orderStore.getAskOrdersByAccount(accountId, from, to);
  }

  public CaretIterator<Ask> getAskOrdersByAccountAsset(final long accountId, final long assetId, int from, int to) {
    return orderStore.getAskOrdersByAccountAsset(accountId, assetId, from, to);
  }

  public CaretIterator<Ask> getSortedAskOrders(long assetId, int from, int to) {
  ...
  public CaretIterator<Bid> getBidOrdersByAccount(long accountId, int from, int to) {
    return orderStore.getBidOrdersByAccount(accountId, from, to);
  }

  public CaretIterator<Bid> getBidOrdersByAccountAsset(final long accountId, final long assetId, int from, int to) {
  ...
  public void addAskOrder(Transaction transaction, Attachment.ColoredCoinsAskOrderPlacement attachment) {
    CaretKey dbKey = askOrderDbKeyFactory.newKey(transaction.getId());
  ...
  public void addBidOrder(Transaction transaction, Attachment.ColoredCoinsBidOrderPlacement attachment) {
    CaretKey dbKey = bidOrderDbKeyFactory.newKey(transaction.getId());
  ...
  private Ask getNextAskOrder(long assetId) {
    return Caret.getStores().getOrderStore().getNextOrder(assetId);
  }

  private Bid getNextBidOrder(long assetId) {
    return Caret.getStores().getOrderStore().getNextBid(assetId);
  }
  ...
  private void matchOrders(long assetId) {
    ...
    while ((askOrder = getNextAskOrder(assetId)) != null
      ...
      Trade trade = tradeService.addTrade(assetId, Caret.getBlockchain().getLastBlock(), askOrder, bidOrder);
