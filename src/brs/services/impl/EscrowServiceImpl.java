...
public class EscrowServiceImpl implements EscrowService {
  ...
  @Override
  public CaretIterator<Escrow> getAllEscrowTransactions() {
  ...
  public boolean isEnabled() {
    if(blockchain.getLastBlock().getHeight() >= Constants.CARET_ESCROW_START_BLOCK) {
      return true;
    }
  ...
  public void removeEscrowTransaction(Long id) {
    ...
    CaretIterator<Decision> decisionIt = escrow.getDecisions();
  ...
  public void addEscrowTransaction(Account sender, Account recipient, Long id, Long amountNQT, int requiredSigners, Collection<Long> signers, int deadline, DecisionType deadlineAction) {
    final CaretKey dbKey = escrowDbKeyFactory.newKey(id);
    ...
    CaretKey senderDbKey = decisionDbKeyFactory.newKey(id, sender.getId());
    ...
    CaretKey recipientDbKey = decisionDbKeyFactory.newKey(id, recipient.getId());
    ...
    for(Long signer : signers) {
      CaretKey signerDbKey = decisionDbKeyFactory.newKey(id, signer);
  ...
  public DecisionType checkComplete(Escrow escrow) {
    ...
    CaretIterator<Decision> decisions = Caret.getStores().getEscrowStore().getDecisions(escrow.getId());
  ...
  public void updateOnBlock(Block block, int blockchainHeight) {
    ...
    CaretIterator<Escrow> deadlineEscrows = escrowTable.getManyBy(getUpdateOnBlockClause(block.getTimestamp()), 0, -1);
    ...
    if (updatedEscrowIds.size() > 0) {
      ...
      if (resultTransactions.size() > 0) {
        Caret.getDbs().getTransactionDb().saveTransactions( resultTransactions);
      }
  ...
  public void saveResultTransaction(Block block, Long escrowId, Long recipientId, Long amountNQT, DecisionType decision, int blockchainHeight) {
    ...
    if(!Caret.getDbs().getTransactionDb().hasTransaction(transaction.getId())) {
