...
var BRS = (function(BRS, $, undefined) {
    BRS.automaticallyCheckRecipient = function() {
        ...
        $recipientFields.on("checkRecipient", function() {
            ...
            if (value && value != "CARET-____-____-____-_____") {
    ...
    BRS.sendMoneyCalculateTotal = function(element) {
        ...
        $(element).closest(".modal").find(".total_amount_ordinary").html(BRS.formatAmount(BRS.convertToNQT(amount + fee)) + " CARET");
    };
            ...
            if (response.publicKey) {
                callback({
                    ...
                    "message": $.t("recipient_info", {
                        "caret": BRS.formatAmount(response.unconfirmedBalanceNQT, false, true)
                    }),
                    "account": response
                });
            } else {
                if (response.errorCode) {
                    if (response.errorCode === 4) {
                        callback({
                            "type": "danger",
                            "message": $.t("recipient_malformed") + (!/^(CARET\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
                            "account": null
                        });
    ...
    BRS.checkRecipient = function(account, modal) {
        ...
        if (/^(CARET\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
