...
var BRS = (function(BRS, $, undefined) {
	...
	BRS.database.select("contacts", null, function(error, contacts) {
		...
		$.each(contacts, function(index, contact) {
		    ...
		    rows += "<tr><td><a href='#' data-toggle='modal' data-target='#update_contact_modal' data-contact='" + String(contact.id).escapeHTML() + "'>" + contact.name.escapeHTML() + "</a></td><td><a href='#' data-user='" + BRS.getAccountFormatted(contact, "account") + "' class='user_info'>" + BRS.getAccountFormatted(contact, "account") + "</a></td><td>" + (contact.email ? contact.email.escapeHTML() : "-") + "</td><td>" + contactDescription.escapeHTML() + "</td><td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_caret") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_message_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("message") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#delete_contact_modal' data-contact='" + String(contact.id).escapeHTML() + "'>" + $.t("delete") + "</a></td></tr>";
		});
	    }
	...
	if (/^\d+$/.test(data.name) || /^CARET\-/i.test(data.name)) {
	    return {
	...
	if (/^CARET\-/i.test(data.account_id)) {
	...
	if (!contactId && BRS.selectedContext) {
	    ...
	    var dbKey = (/^CARET\-/i.test(accountId) ? "accountRS" : "account");
