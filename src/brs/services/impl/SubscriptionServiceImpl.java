...
public class SubscriptionServiceImpl implements SubscriptionService {
  ...
  public void addSubscription(Account sender, Account recipient, Long id, Long amountNQT, int startTimestamp, int frequency) {
    final CaretKey dbKey = subscriptionDbKeyFactory.newKey(id);
  ...
  public boolean isEnabled() {
    if (blockchain.getLastBlock().getHeight() >= Constants.CARET_SUBSCRIPTION_START_BLOCK) {
      return true;
    }
  ...
  public long calculateFees(int timestamp) {
    long totalFeeNQT = 0;
    CaretIterator<Subscription> updateSubscriptions = subscriptionStore.getUpdateSubscriptions(timestamp);
