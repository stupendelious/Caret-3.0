...
public class TransactionServiceImpl implements TransactionService {
  ...
  public void validate(Transaction transaction) throws CaretException.ValidationException {
    ...
    if (transaction.getFeeNQT() < minimumFeeNQT) {
      throw new CaretException.NotCurrentlyValidException(String.format("Transaction fee %d less than minimum fee %d at height %d",
    ...
    if (blockchain.getHeight() >= Constants.PUBLIC_KEY_ANNOUNCEMENT_BLOCK) {
      if (transaction.getType().hasRecipient() && transaction.getRecipientId()!= 0) {
        ...
        if ((recipientAccount == null || recipientAccount.getPublicKey() == null) && transaction.getPublicKeyAnnouncement() == null) {
          throw new CaretException.NotCurrentlyValidException("Recipient account does not have a public key, must attach a public key announcement");
        }
      }
    }
  }
