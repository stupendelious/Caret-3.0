...
/**
 * Java LogManager extension for use with Caret
 */
public class CaretLogManager extends LogManager {

  /** Logging reconfiguration in progress */
  private volatile boolean loggingReconfiguration = false;

  /**
   * Create the Caret log manager
   *
   ...
   * done terminating the Caret processes.
   */
  public CaretLogManager() {
    super();
  }
   ...
   * This allows us to continue to use logging facilities during Caret shutdown.
   */
  @Override
  public void reset() {
  ...
  /**
   * Caret shutdown is now complete, so call LogManager.reset() to terminate
   * the log handlers.
   */
  void caretShutdown() {
    super.reset();
