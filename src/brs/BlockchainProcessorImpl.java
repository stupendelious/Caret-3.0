...
public final class BlockchainProcessorImpl implements BlockchainProcessor {
         ...
         //   List<Block> forkBlocks = new ArrayList<>();
            ...
            for (Object o : nextBlocks) {
              ...
              try {
                ...
                downloadCache.resetCache();
                return;
              } catch (RuntimeException | CaretException.ValidationException e) {
          ...
          } catch (CaretException.StopException e) {
  ...
  @Override
  public void processPeerBlock(JSONObject request, Peer peer) throws CaretException {
    ...
    synchronized (transactionProcessor.getUnconfirmedTransactionsSyncObj()) {
      ...
      try {
        ...
        for (Transaction transaction : block.getTransactions()) {
          ...
          if (Caret.getFluxCapacitor().isActive(FeatureToggle.AUTOMATED_TRANSACTION_BLOCK)) {
  ...
  private List<Block> popOffTo(Block commonBlock) {
    ...
    synchronized (downloadCache) {
      synchronized (transactionProcessor.getUnconfirmedTransactionsSyncObj()) {
        //Caret.getTransactionProcessor().clearUnconfirmedTransactions();
        try {
          stores.beginTransaction();
  ...
  public void generateBlock(String secretPhrase, byte[] publicKey, Long nonce) throws BlockNotAcceptedException {
    synchronized (downloadCache) {
      ...
      int blockSize   = Caret.getFluxCapacitor().getInt(FluxInt.MAX_NUMBER_TRANSACTIONS);
      int payloadSize = Caret.getFluxCapacitor().getInt(FluxInt.MAX_PAYLOAD_LENGTH);
      ...
      try {
        ...
        List<Transaction> unconfirmedTransactionsOrderedByFee = unconfirmedTransactionStore.getAll().stream().filter(
            transaction ->
              transaction.getVersion() == transactionProcessor.getTransactionVersion(previousBlock.getHeight())
                  ...
                  && (
                      ! Caret.getFluxCapacitor().isActive(FeatureToggle.AUTOMATED_TRANSACTION_BLOCK)
        ...
        COLLECT_TRANSACTIONS: for (Transaction transaction : unconfirmedTransactionsOrderedByFee) {
          ...
          while ( ! transactionHasBeenHandled ) {
            ...
            Long slotFee = Caret.getFluxCapacitor().isActive(PRE_DYMAXION) ? blockSize * FEE_QUANT : ONE_CARET;
            if (transaction.getFeeNQT() >= slotFee) {
              ...
              if (hasAllReferencedTransactions(transaction, transaction.getTimestamp(), 0)) {
                ...
                if (! transactionDuplicatesChecker.hasAnyDuplicate(transaction) && ! transactionDb.hasTransaction(transaction.getId()) && transactionService.applyUnconfirmed(transaction)) {
                  ...
                  } catch (CaretException.NotCurrentlyValidException e) {
      ...
      try {
        block = new Block(getBlockVersion(), blockTimestamp,
            previousBlock.getId(), totalAmountNQT, totalFeeNQT, Caret.getFluxCapacitor().getInt(FluxInt.MAX_PAYLOAD_LENGTH) - payloadSize, payloadHash, publicKey,
