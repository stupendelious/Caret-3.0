...
public class Account {
  ...
  public static class AccountAsset {
    ...
    public final CaretKey caretKey;
    private long quantityQNT;
    private long unconfirmedQuantityQNT;

    protected AccountAsset(long accountId, long assetId, long quantityQNT, long unconfirmedQuantityQNT, CaretKey caretKey) {
    ...
    public AccountAsset(CaretKey caretKey, long accountId, long assetId, long quantityQNT, long unconfirmedQuantityQNT) {
      this.accountId = accountId;
      this.assetId = assetId;
      this.caretKey = caretKey;
      this.quantityQNT = quantityQNT;
  ...
  public static class RewardRecipientAssignment {
    ...
    public RewardRecipientAssignment(Long accountId, Long prevRecipientId, Long recipientId, int fromHeight, CaretKey caretKey) {
      this.accountId = accountId;
  ...
  protected static final CaretKey.LongKeyFactory<Account> accountCaretKeyFactory() {
    return Caret.getStores().getAccountStore().getAccountKeyFactory();
  }

  private static final VersionedBatchEntityTable<Account> accountTable() {
    return Caret.getStores().getAccountStore().getAccountTable();
  }
  ...
  public Account(long id) {
    ...
    this.id = id;
    this.nxtKey = accountCaretKeyFactory().newKey(this.id);
    this.creationHeight = Caret.getBlockchain().getHeight();
  }

  protected Account(long id, CaretKey caretKey, int creationHeight) {
    if (id != Crypto.rsDecode(Crypto.rsEncode(id))) {
      logger.log(Level.INFO, "CRITICAL ERROR: Reed-Solomon encoding fails for {0}", id);
    }
    this.id = id;
    this.nxtKey = caretKey;
  ...
  public boolean setOrVerify(byte[] key, int height) {
    return Caret.getStores().getAccountStore().setOrVerify(this, key, height);
  }
