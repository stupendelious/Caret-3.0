...
var BRS = (function(BRS, $, undefined) {
    ...
    $('#send_money_modal').on('hide.bs.modal', function (e) {
           $("#total_amount_multi_out").html('0.1 CARET');
        });
  ...
  var accountButton;
    ...
    var type;
		...
		if (type == $.t("alias_sale")) {
			...
			if (!response.errorCode) {
				...
				message = $.t("alias_sale_direct_offer", {
				    "caret": BRS.formatAmount(transaction.attachment.priceNQT)
		...
		BRS.sendRequest("getAsset", {
			...
			"asset_name": asset.name,
			"quantity": [transaction.attachment.quantityQNT, asset.decimals],
			"price_formatted_html": BRS.formatOrderPricePerWholeQNT(transaction.attachment.priceNQT, asset.decimals) + " CARET",
			"total_formatted_html": BRS.formatAmount(BRS.calculateOrderTotalNQT(transaction.attachment.quantityQNT, transaction.attachment.priceNQT)) + " CARET"
		    };
		...
		BRS.sendRequest("getDGSGood", {
			...
			"item_name": goods.name,
			"new_price_formatted_html": BRS.formatAmount(transaction.attachment.priceNQT) + " CARET",
			"seller": BRS.getAccountFormatted(goods, "seller")
		    };
		...
		BRS.sendRequest("getDGSPurchase", {
			...
			if (purchase.quantity != "1") {
			    ...
			    data.total_formatted_html = orderTotal + " CARET";
			}
		...
		BRS.sendRequest("getDGSPurchase", {
			...
			data.order_total_formatted_html = BRS.formatAmount(orderTotal) + " CARET";
