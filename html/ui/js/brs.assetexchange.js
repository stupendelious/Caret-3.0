...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.forms.addAssetBookmark = function($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^CARET\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^CARET\-/i.test(data.id)) {
    ...
    BRS.loadAsset = function(asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_caret").html($.t("sell_asset_for_caret", {
                "assetName": String(asset.name).escapeHTML()
            }));
            $("#buy_asset_with_caret").html($.t("buy_asset_with_caret", {
                "assetName": String(asset.name).escapeHTML()
            }));
        ...
        if (BRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_caret_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_caret_balance").html(BRS.formatAmount(BRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_order_modal").on("show.bs.modal", function(e) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "caret": BRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "caret": BRS.formatAmount(priceNQTPerWholeQNT, false, true),
                "total_caret": totalNXT
            });
        } else {
        ...
        $("#asset_order_total").html(totalNXT + " CARET");
        $("#asset_order_fee_paid").html(BRS.formatAmount(feeNQT) + " CARET");
