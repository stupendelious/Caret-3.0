...
you can activate your MainNet config with "caret.sh switch MainNet" 
...
function upgrade_conf () {
    ...
    then
        ...
        BRS="${BRS//caret\.rebroadcastPeers/P2P.rebroadcastTo}"
        BRS="${BRS//caret\.connectWellKnownFirst/P2P.NumBootstrapConnections}"
        ...
        BRS="${BRS//caret\.rebroadcastAfter/P2P.rebroadcastTxAfter}"
        BRS="${BRS//caret\.rebroadcastEvery/P2P.rebroadcastTxEvery}"
        ...
        BRS="${BRS//caret\.mockMining/DEV.mockMining}"
        ...
        # GPU-related params
        BRS="${BRS//caret\.oclVerify/GPU.Acceleration}"
        BRS="${BRS//caret\.oclAuto/GPU.AutoDetect}"
        BRS="${BRS//caret\.oclPlatform/GPU.PlatformIdx}"
        BRS="${BRS//caret\.oclDevice/GPU.DeviceIdx}"
        BRS="${BRS//caret\.oclMemPercent/GPU.MemPercent}"
        BRS="${BRS//caret\.oclHashesPerEnqueue/GPU.HashesPerBatch}"
...
function create_brs_db {
    ...
    echo "[+] Creating caret wallet db ($P_DATA)..."
...
function exists_or_get {
    if [ -f $1 ]; then
        ...
        wget https://download.cryptoguru.org/caret/wallet/$1
    fi
}
...
if [[ $# -gt 0 ]] ; then
    case "$MY_CMD" in
        ...
        "compile")
            ...
            ;;
        "h2shell")
            java -cp caret.jar org.h2.tools.Shell
            ;;
        "import")
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                if [[ $MY_ARG == "mariadb" ]]; then
                    ...
                    if exists_or_get brs.h2.zip ; then
                        mkdir -p "$MY_DIR/caret_db"
                        rm -f caret_db/caret.trace.db
                        if unzip brs.h2.zip ; then
                            if mv caret.mv.db "$MY_DIR/caret_db"; then
    ...
    java $BRS_DEVSTART -cp caret.jar:conf brs.Caret
fi
