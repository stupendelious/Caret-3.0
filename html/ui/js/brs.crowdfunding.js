...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.pages.crowdfunding = function() {
        ...
        if (!isLoaded) {
            isLoaded = true;
            $.ajax({
                url: '//' + window.location.hostname.toLowerCase() + myPort + '/caret',
                type: 'POST',
                dataType: "json",
    ...
    $('#deploy-at-btn').on('click', function(e) {
        ...
        var weeksA = $('#sel1').val();
        var caretsA = $('#funding-amount').val();
        caretsA = caretsA * 100000000;
        var passA = $('#funding-password').val();
        var feeA = 1000000000;
        var minA = 700000000;
        var hexA = Number(caretsA).toString(16).toLowerCase();
        var finalA = '';
        if ((hexA.length % 2) !== 0) {
            hexA = "0" + hexA;
        }
