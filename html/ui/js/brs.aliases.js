...
BRS = (function (BRS, $, undefined) {
        ...
        if (alias) {
            ...
            BRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    else if ((aliasURI = /acct:(.*)@caret/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    BRS.forms.setAlias = function ($modal) {
        ...
        if (data.type === "account") {
            if (!(/acct:(.*)@caret/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(CARET\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@caret";
                    }
                }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            $("#register_alias_uri").val("").mask("CARET-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@caret/i);
                if (!match) {
                ...
                else if (!/^CARET\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = BRS.accountRS;
                }
                    ...
                    if ("priceNQT" in response) {
                        if (response.buyer == BRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "caret": BRS.formatAmount(response.priceNQT)
