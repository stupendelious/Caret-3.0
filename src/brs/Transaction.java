...
public class Transaction implements Comparable<Transaction> {
  ...
  public static class Builder {
    ...
    public Transaction build() throws CaretException.NotValidException {
  ...
  private volatile CaretKey dbKey;

  private Transaction(Builder builder) throws CaretException.NotValidException {
    ...
    int effectiveHeight = (height < Integer.MAX_VALUE ? height : Caret.getBlockchain().getHeight());
    ...
    if(type == null || type.isSigned()) {
      if (builder.feeNQT > 0 && builder.feeNQT < minimumFeeNQT) {
        throw new CaretException.NotValidException(String.format("Requested fee %d less than the minimum fee %d",
    ...
    if(type == null || type.isSigned()) {
      if (deadline < 1
          ...
          || type == null) {
        throw new CaretException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                                                 + ", deadline: " + deadline + ", fee: " + feeNQT + ", amount: " + amountNQT);
      }
    }

    if (attachment == null || type != attachment.getTransactionType()) {
      throw new CaretException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
    }
    ...
    if (! type.hasRecipient() && attachment.getTransactionType() != Payment.MULTI_OUT && attachment.getTransactionType() != Payment.MULTI_SAME_OUT) {
      if (recipientId != 0 || getAmountNQT() != 0) {
        throw new CaretException.NotValidException("Transactions of this type must have recipient == Genesis, amount == 0");
      }
    }

    for (Appendix.AbstractAppendix appendage : appendages) {
      if (! appendage.verifyVersion(this.version)) {
        throw new CaretException.NotValidException("Invalid attachment version " + appendage.getVersion()
  ...
  public byte[] getBytes() {
    try {
      ...
      if(type.isSigned() || ! Caret.getFluxCapacitor().isActive(FeatureToggle.AT_FIX_BLOCK_4)) {
      ...
      if (useNQT()) {
        ...
        buffer.putInt((int) (amountNQT / Constants.ONE_CARET));
        buffer.putInt((int) (feeNQT / Constants.ONE_CARET));
        if (referencedTransactionFullHash != null) {
  ...
  public static Transaction parseTransaction(byte[] bytes) throws CaretException.ValidationException {
    try {
    ...
    } catch (CaretException.NotValidException|RuntimeException e) {
  ...
  */
  ...
  static Transaction parseTransaction(JSONObject transactionData, int height) throws CaretException.NotValidException {
    try {
      ...
      if (transactionType == null) {
        throw new CaretException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
      }
  ...
  private boolean useNQT() {
    return this.height > Constants.NQT_BLOCK
        && (this.height < Integer.MAX_VALUE
            || Caret.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
  }
