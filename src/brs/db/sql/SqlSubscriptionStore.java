...
public class SqlSubscriptionStore implements SubscriptionStore {
  ...
  private final CaretKey.LongKeyFactory<Subscription> subscriptionDbKeyFactory = new DbKey.LongKeyFactory<Subscription>("id") {
      @Override
      public CaretKey newKey(Subscription subscription) {
  ...
  @Override
  public CaretKey.LongKeyFactory<Subscription> getSubscriptionDbKeyFactory() {
  ...
  @Override
  public CaretIterator<Subscription> getSubscriptionsByParticipant(Long accountId) {
    return subscriptionTable.getManyBy(getByParticipantClause(accountId), 0, -1);
  }

  @Override
  public CaretIterator<Subscription> getIdSubscriptions(Long accountId) {
    return subscriptionTable.getManyBy(SUBSCRIPTION.SENDER_ID.eq(accountId), 0, -1);
  }

  @Override
  public CaretIterator<Subscription> getSubscriptionsToId(Long accountId) {
    return subscriptionTable.getManyBy(SUBSCRIPTION.RECIPIENT_ID.eq(accountId), 0, -1);
  }

  @Override
  public CaretIterator<Subscription> getUpdateSubscriptions(int timestamp) {
  ...
  protected void saveSubscription(DSLContext ctx, Subscription subscription) throws SQLException {
    ...
    subscriptionRecord.setHeight(brs.Caret.getBlockchain().getHeight());
