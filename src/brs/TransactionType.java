...
import static brs.Constants.ONE_CARET;
...
import brs.CaretException.NotValidException;
...
public abstract class TransactionType {
  ...
  private static final byte TYPE_CARET_MINING = 20; // jump some for easier nxt updating
  ...
  private static final byte SUBTYPE_CARET_MINING_REWARD_RECIPIENT_ASSIGNMENT = 0;
  ...
  public static TransactionType findTransactionType(byte type, byte subtype) {
    switch (type) {
      ...
      case TYPE_CARET_MINING:
        switch (subtype) {
          case SUBTYPE_CARET_MINING_REWARD_RECIPIENT_ASSIGNMENT:
            return CaretMining.REWARD_RECIPIENT_ASSIGNMENT;
          default:
            return null;
        }
  ...
  public abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException;

  abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws CaretException.NotValidException;

  abstract void validateAttachment(Transaction transaction) throws CaretException.ValidationException;
  ...
  public void parseAppendices(Transaction.Builder builder, JSONObject attachmentData) throws CaretException.NotValidException {
  ...
  public void parseAppendices(Transaction.Builder builder, int flags, byte version, ByteBuffer buffer) throws CaretException.ValidationException {
    int position = 1;
    if ((flags & position) != 0) {
  ...
  public static abstract class Payment extends TransactionType {
    ...
    public static final TransactionType ORDINARY = new Payment() {
        ...
        @Override
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
            throw new CaretException.NotValidException("Invalid ordinary payment");
          }
        }
    ...
    public static final TransactionType MULTI_OUT = new Payment() {
      ...
      @Override
      public Attachment.PaymentMultiOutCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        return new Attachment.PaymentMultiOutCreation(buffer, transactionVersion);
      }

      @Override
      Attachment.PaymentMultiOutCreation parseAttachment(JSONObject attachmentData) throws CaretException.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
        if (! fluxCapacitor.isActive(FeatureToggle.PRE_DYMAXION) ) {
          throw new CaretException.NotCurrentlyValidException("Multi Out Payments are not allowed before the Pre Dymaxion block");
        }
        ...
        if (attachment.getAmountNQT() <= 0
         ...
         || attachment.getRecipients().size() < 2) {
          throw new CaretException.NotValidException("Invalid multi out payment");
        }
      }
    ...
    public static final TransactionType MULTI_SAME_OUT = new Payment() {
      ...
      @Override
      public Attachment.PaymentMultiSameOutCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        return new Attachment.PaymentMultiSameOutCreation(buffer, transactionVersion);
      }

      @Override
      Attachment.PaymentMultiSameOutCreation parseAttachment(JSONObject attachmentData) throws CaretException.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
        if (! fluxCapacitor.isActive(FeatureToggle.PRE_DYMAXION) ) {
          throw new CaretException.NotCurrentlyValidException("Multi Same Out Payments are not allowed before the Pre Dymaxion block");
        }
  ...
  public static abstract class Messaging extends TransactionType {
    ...
    public static final TransactionType ARBITRARY_MESSAGE = new Messaging() {
      ...
      void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
        ...
        if (transaction.getAmountNQT() != 0) {
          throw new CaretException.NotValidException("Invalid arbitrary message: " + attachment.getJSONObject());
        }
        if (! fluxCapacitor.isActive(FeatureToggle.DIGITAL_GOODS_STORE) && transaction.getMessage() == null) {
          throw new CaretException.NotCurrentlyValidException("Missing message appendix not allowed before DGS block");
        }
      }
    ...
    public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
        ...
        @Override
        public Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getAliasName().isEmpty()
            ...
            throw new CaretException.NotValidException("Invalid alias assignment: " + attachment.getJSONObject());
          }
          String normalizedAlias = attachment.getAliasName().toLowerCase();
          for (int i = 0; i < normalizedAlias.length(); i++) {
            if (Constants.ALPHABET.indexOf(normalizedAlias.charAt(i)) < 0) {
              throw new CaretException.NotValidException("Invalid alias name: " + normalizedAlias);
            }
          }
          Alias alias = aliasService.getAlias(normalizedAlias);
          if (alias != null && alias.getAccountId() != transaction.getSenderId()) {
            throw new CaretException.NotCurrentlyValidException("Alias already owned by another account: " + normalizedAlias);
          }
        }
    ...
    public static final TransactionType ALIAS_SELL = new Messaging() {
        ...
        @Override
        public Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          if (! fluxCapacitor.isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
            throw new CaretException.NotYetEnabledException("Alias transfer not yet enabled at height " + blockchain.getLastBlock().getHeight());
          }
          if (transaction.getAmountNQT() != 0) {
            throw new CaretException.NotValidException("Invalid sell alias transaction: " +
          ...
          if (aliasName == null || aliasName.isEmpty()) {
            throw new CaretException.NotValidException("Missing alias name");
          }
          long priceNQT = attachment.getPriceNQT();
          if (priceNQT < 0 || priceNQT > Constants.MAX_BALANCE_NQT) {
            throw new CaretException.NotValidException("Invalid alias sell price: " + priceNQT);
          }
          if (priceNQT == 0) {
            if (Genesis.CREATOR_ID == transaction.getRecipientId()) {
              throw new CaretException.NotValidException("Transferring aliases to Genesis account not allowed");
            } else if (transaction.getRecipientId() == 0) {
              throw new CaretException.NotValidException("Missing alias transfer recipient");
            }
          }
          final Alias alias = aliasService.getAlias(aliasName);
          if (alias == null) {
            throw new CaretException.NotCurrentlyValidException("Alias hasn't been registered yet: " + aliasName);
          } else if (alias.getAccountId() != transaction.getSenderId()) {
            throw new CaretException.NotCurrentlyValidException("Alias doesn't belong to sender: " + aliasName);
          }
        }
    ...
    public static final TransactionType ALIAS_BUY = new Messaging() {
        ...
        @Override
        public Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (alias == null) {
            ...
            throw new CaretException.NotCurrentlyValidException("Alias is owned by account other than recipient: "
                                                              + Convert.toUnsignedLong(alias.getAccountId()));
          }
          Alias.Offer offer = aliasService.getOffer(alias);
          if (offer == null) {
            throw new CaretException.NotCurrentlyValidException("Alias is not for sale: " + aliasName);
          }
          if (transaction.getAmountNQT() < offer.getPriceNQT()) {
            ...
            throw new CaretException.NotCurrentlyValidException(msg);
          }
          if (offer.getBuyerId() != 0 && offer.getBuyerId() != transaction.getSenderId()) {
            throw new CaretException.NotCurrentlyValidException("Wrong buyer for " + aliasName + ": "
    ...
    public static final Messaging ACCOUNT_INFO = new Messaging() {
        ...
        @Override
        public Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (Convert.toBytes(attachment.getName()).length > Constants.MAX_ACCOUNT_NAME_LENGTH
            ...
            throw new CaretException.NotValidException("Invalid account info issuance: " + attachment.getJSONObject());
          }
        }
  ...
  public static abstract class ColoredCoins extends TransactionType {
    ...
    public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
        ...
        @Override
        public Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getName().length() < Constants.MIN_ASSET_NAME_LENGTH
            ...
            throw new CaretException.NotValidException("Invalid asset issuance: " + attachment.getJSONObject());
          }
          String normalizedName = attachment.getName().toLowerCase();
          for (int i = 0; i < normalizedName.length(); i++) {
            if (Constants.ALPHABET.indexOf(normalizedName.charAt(i)) < 0) {
              throw new CaretException.NotValidException("Invalid asset name: " + normalizedName);
            }
          }
        }
    ...
    public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
        ...
        @Override
        public Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (transaction.getAmountNQT() != 0
            ...
            throw new CaretException.NotValidException("Invalid asset transfer amount or comment: " + attachment.getJSONObject());
          }
          if (transaction.getVersion() > 0 && attachment.getComment() != null) {
            throw new CaretException.NotValidException("Asset transfer comments no longer allowed, use message " +
          ...
          if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
            throw new CaretException.NotValidException("Invalid asset transfer asset or quantity: " + attachment.getJSONObject());
          }
          if (asset == null) {
            throw new CaretException.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) +
                                                              " does not exist yet");
          }
        }
    ...
    abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
      ...
      @Override
      final void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
        Attachment.ColoredCoinsOrderPlacement attachment = (Attachment.ColoredCoinsOrderPlacement)transaction.getAttachment();
        if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
            || attachment.getAssetId() == 0) {
          throw new CaretException.NotValidException("Invalid asset order placement: " + attachment.getJSONObject());
        }
        Asset asset = assetExchange.getAsset(attachment.getAssetId());
        if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
          throw new CaretException.NotValidException("Invalid asset order placement asset or quantity: " + attachment.getJSONObject());
        }
        if (asset == null) {
          throw new CaretException.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) +
                                                            " does not exist yet");
    ...
    public static final TransactionType ASK_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (ask == null) {
            throw new CaretException.NotCurrentlyValidException("Invalid ask order: " + Convert.toUnsignedLong(attachment.getOrderId()));
          }
          if (ask.getAccountId() != transaction.getSenderId()) {
            throw new CaretException.NotValidException("Order " + Convert.toUnsignedLong(attachment.getOrderId()) + " was created by account "
    ...
    public static final TransactionType BID_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (bid == null) {
            throw new CaretException.NotCurrentlyValidException("Invalid bid order: " + Convert.toUnsignedLong(attachment.getOrderId()));
          }
  ...
  public static abstract class DigitalGoods extends TransactionType {
    ...
    final void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
      if (! fluxCapacitor.isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
        throw new CaretException.NotYetEnabledException("Digital goods listing not yet enabled at height " + blockchain.getLastBlock().getHeight());
      }
      if (transaction.getAmountNQT() != 0) {
        throw new CaretException.NotValidException("Invalid digital goods transaction");
      }
      doValidateAttachment(transaction);
    }

    abstract void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException;
    ...
    public static final TransactionType LISTING = new DigitalGoods() {
        ...
        @Override
        public Attachment.DigitalGoodsListing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        @Override
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          Attachment.DigitalGoodsListing attachment = (Attachment.DigitalGoodsListing) transaction.getAttachment();
          if (attachment.getName().isEmpty()
            ...
            throw new CaretException.NotValidException("Invalid digital goods listing: " + attachment.getJSONObject());
          }
        }
    ...
    public static final TransactionType DELISTING = new DigitalGoods() {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (goods != null && transaction.getSenderId() != goods.getSellerId()) {
            throw new CaretException.NotValidException("Invalid digital goods delisting - seller is different: " + attachment.getJSONObject());
          }
          if (goods == null || goods.isDelisted()) {
            throw new CaretException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
    ...
    public static final TransactionType PRICE_CHANGE = new DigitalGoods() {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
            ...
            throw new CaretException.NotValidException("Invalid digital goods price change: " + attachment.getJSONObject());
          }
          if (goods == null || goods.isDelisted()) {
            throw new CaretException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                                                              "not yet listed or already delisted");
    ...
    public static final TransactionType QUANTITY_CHANGE = new DigitalGoods() {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getDeltaQuantity() < -Constants.MAX_DGS_LISTING_QUANTITY
              ...
              || (goods != null && transaction.getSenderId() != goods.getSellerId())) {
            throw new CaretException.NotValidException("Invalid digital goods quantity change: " + attachment.getJSONObject());
          }
          if (goods == null || goods.isDelisted()) {
            throw new CaretException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                                                              "not yet listed or already delisted");
    ...
    public static final TransactionType PURCHASE = new DigitalGoods() {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getQuantity() <= 0 || attachment.getQuantity() > Constants.MAX_DGS_LISTING_QUANTITY
            ...
            throw new CaretException.NotValidException("Invalid digital goods purchase: " + attachment.getJSONObject());
          }
          if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
            throw new CaretException.NotValidException("Only text encrypted messages allowed");
          }
          if (goods == null || goods.isDelisted()) {
            throw new CaretException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                                                              "not yet listed or already delisted");
          ...
          if (attachment.getQuantity() > goods.getQuantity() || attachment.getPriceNQT() != goods.getPriceNQT()) {
            throw new CaretException.NotCurrentlyValidException("Goods price or quantity changed: " + attachment.getJSONObject());
          }
          if (attachment.getDeliveryDeadlineTimestamp() <= blockchain.getLastBlock().getTimestamp()) {
            throw new CaretException.NotCurrentlyValidException("Delivery deadline has already expired: " + attachment.getDeliveryDeadlineTimestamp());
          }
        }
    ...
    public static final TransactionType DELIVERY = new DigitalGoods() {
        ...
        @Override
        public Attachment.DigitalGoodsDelivery parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getGoods().getData().length > Constants.MAX_DGS_GOODS_LENGTH
            ...
            throw new CaretException.NotValidException("Invalid digital goods delivery: " + attachment.getJSONObject());
          }
          if (purchase == null || purchase.getEncryptedGoods() != null) {
            throw new CaretException.NotCurrentlyValidException("Purchase does not exist yet, or already delivered: "
    ...
    public static final TransactionType FEEDBACK = new DigitalGoods() {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (purchase != null &&
            ...
            throw new CaretException.NotValidException("Invalid digital goods feedback: " + attachment.getJSONObject());
          }
          if (transaction.getEncryptedMessage() == null && transaction.getMessage() == null) {
            throw new CaretException.NotValidException("Missing feedback message");
          }
          if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
            throw new CaretException.NotValidException("Only text encrypted messages allowed");
          }
          if (transaction.getMessage() != null && ! transaction.getMessage().isText()) {
            throw new CaretException.NotValidException("Only text public messages allowed");
          }
          if (purchase == null || purchase.getEncryptedGoods() == null) {
            throw new CaretException.NotCurrentlyValidException("Purchase does not exist yet or not yet delivered");
          }
        }
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getRefundNQT() < 0 || attachment.getRefundNQT() > Constants.MAX_BALANCE_NQT
              || (purchase != null &&
                  (purchase.getBuyerId() != transaction.getRecipientId()
                   || transaction.getSenderId() != purchase.getSellerId()))) {
            throw new CaretException.NotValidException("Invalid digital goods refund: " + attachment.getJSONObject());
          }
          if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
            throw new CaretException.NotValidException("Only text encrypted messages allowed");
          }
          if (purchase == null || purchase.getEncryptedGoods() == null || purchase.getRefundNQT() != 0) {
            throw new CaretException.NotCurrentlyValidException("Purchase does not exist or is not delivered or is already refunded");
          }
        }
    ...
    public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (transaction.getSenderId() == transaction.getRecipientId()
            ...
            throw new CaretException.NotValidException("Invalid effective balance leasing: "
          ...
          if (recipientAccount == null
            ...
            throw new CaretException.NotCurrentlyValidException("Invalid effective balance leasing: "
  ...
  public static abstract class CaretMining extends TransactionType {

    private CaretMining() {}

    @Override
    public final byte getType() {
      return TransactionType.TYPE_CARET_MINING;
    }
    ...
    public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new CaretMining() {

        @Override
        public final byte getSubtype() {
          return TransactionType.SUBTYPE_CARET_MINING_REWARD_RECIPIENT_ASSIGNMENT;
        }

        @Override
        public Attachment.CaretMiningRewardRecipientAssignment
        parseAttachment(ByteBuffer buffer, byte transactionVersion) {
          return new Attachment.CaretMiningRewardRecipientAssignment(buffer, transactionVersion);
        }

        @Override
        Attachment.CaretMiningRewardRecipientAssignment parseAttachment(JSONObject attachmentData) {
          return new Attachment.CaretMiningRewardRecipientAssignment(attachmentData);
        }
        ...
        public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
          ...
          return new TransactionDuplicationKey(CaretMining.REWARD_RECIPIENT_ASSIGNMENT, Convert.toUnsignedLong(transaction.getSenderId()));
        }
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (sender == null) {
            throw new CaretException.NotCurrentlyValidException("Sender not yet known ?!");
          }
          ...
          if (rewardAssignment != null && rewardAssignment.getFromHeight() >= height) {
            throw new CaretException.NotCurrentlyValidException("Cannot reassign reward recipient before previous goes into effect: "
                                                                + transaction.getJSONObject());
          }
          Account recip = accountService.getAccount(transaction.getRecipientId());
          if (recip == null || recip.getPublicKey() == null) {
            throw new CaretException.NotValidException("Reward recipient must have public key saved in blockchain: "
                                                       + transaction.getJSONObject());
          }

          if (fluxCapacitor.isActive(PRE_DYMAXION)) {
            if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() < FEE_QUANT) {
              throw new CaretException.NotValidException("Reward recipient assignment transaction must have 0 send amount and at least minimum fee: "
                      + transaction.getJSONObject());
            }
          } else {
            if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_CARET) {
              throw new CaretException.NotValidException("Reward recipient assignment transaction must have 0 send amount and 1 fee: "
                      + transaction.getJSONObject());
            }
          }

          if (height < Constants.CARET_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK) {
            throw new CaretException.NotCurrentlyValidException("Reward recipient assignment not allowed before block "
                                                                + Constants.CARET_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK);
          }
        }
  ...
  public static abstract class AdvancedPayment extends TransactionType {
    ...
    public static final TransactionType ESCROW_CREATION = new AdvancedPayment() {
        ...
        @Override
        public Attachment.AdvancedPaymentEscrowCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
          return new Attachment.AdvancedPaymentEscrowCreation(buffer, transactionVersion);
        }

        @Override
        Attachment.AdvancedPaymentEscrowCreation parseAttachment(JSONObject attachmentData) throws CaretException.NotValidException {
        ...
        public Long calculateAttachmentTotalAmountNQT(Transaction transaction) {
          ...
          return Convert.safeAdd(attachment.getAmountNQT(), Convert.safeMultiply(attachment.getTotalSigners(), Constants.ONE_CARET));
        }
        ...
        final void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
          ...
          signers.forEach(signer -> accountService.addToBalanceAndUnconfirmedBalanceNQT(accountService.getOrAddAccount(signer), Constants.ONE_CARET));
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (transaction.getSenderId() == transaction.getRecipientId()) {
            throw new CaretException.NotValidException("Escrow must have different sender and recipient");
          }
          totalAmountNQT = Convert.safeAdd(totalAmountNQT, attachment.getTotalSigners() * Constants.ONE_CARET);
          if (transaction.getAmountNQT() != 0) {
            throw new CaretException.NotValidException("Transaction sent amount must be 0 for escrow");
          }
          if (totalAmountNQT.compareTo(0L) < 0 ||
             totalAmountNQT.compareTo(Constants.MAX_BALANCE_NQT) > 0)
            {
              throw new CaretException.NotValidException("Invalid escrow creation amount");
            }
          if (transaction.getFeeNQT() < Constants.ONE_CARET) {
            throw new CaretException.NotValidException("Escrow transaction must have a fee at least 1 caret");
          }
          if (attachment.getRequiredSigners() < 1 || attachment.getRequiredSigners() > 10) {
            throw new CaretException.NotValidException("Escrow required signers much be 1 - 10");
          }
          if (attachment.getRequiredSigners() > attachment.getTotalSigners()) {
            throw new CaretException.NotValidException("Cannot have more required than signers on escrow");
          }
          if (attachment.getTotalSigners() < 1 || attachment.getTotalSigners() > 10) {
            throw new CaretException.NotValidException("Escrow transaction requires 1 - 10 signers");
          }
          if (attachment.getDeadline() < 1 || attachment.getDeadline() > 7776000) { // max deadline 3 months
            throw new CaretException.NotValidException("Escrow deadline must be 1 - 7776000 seconds");
          }
          if (attachment.getDeadlineAction() == null || attachment.getDeadlineAction() == Escrow.DecisionType.UNDECIDED) {
            throw new CaretException.NotValidException("Invalid deadline action for escrow");
          }
          if (attachment.getSigners().contains(transaction.getSenderId()) ||
             attachment.getSigners().contains(transaction.getRecipientId())) {
            throw new CaretException.NotValidException("Escrow sender and recipient cannot be signers");
          }
          if (!escrowService.isEnabled()) {
            throw new CaretException.NotYetEnabledException("Escrow not yet enabled");
          }
        }
    ...
    public static final TransactionType ESCROW_SIGN = new AdvancedPayment() {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          Attachment.AdvancedPaymentEscrowSign attachment = (Attachment.AdvancedPaymentEscrowSign) transaction.getAttachment();
          if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_CARET) {
            throw new CaretException.NotValidException("Escrow signing must have amount 0 and fee of 1");
          }
          if (attachment.getEscrowId() == null || attachment.getDecision() == null) {
            throw new CaretException.NotValidException("Escrow signing requires escrow id and decision set");
          }
          Escrow escrow = escrowService.getEscrowTransaction(attachment.getEscrowId());
          if (escrow == null) {
            throw new CaretException.NotValidException("Escrow transaction not found");
          }
          if (!escrowService.isIdSigner(transaction.getSenderId(), escrow) &&
            ...
            throw new CaretException.NotValidException("Sender is not a participant in specified escrow");
          }
          if (escrow.getSenderId().equals(transaction.getSenderId()) && attachment.getDecision() != Escrow.DecisionType.RELEASE) {
            throw new CaretException.NotValidException("Escrow sender can only release");
          }
          if (escrow.getRecipientId().equals(transaction.getSenderId()) && attachment.getDecision() != Escrow.DecisionType.REFUND) {
            throw new CaretException.NotValidException("Escrow recipient can only refund");
          }
          if (!escrowService.isEnabled()) {
            throw new CaretException.NotYetEnabledException("Escrow not yet enabled");
          }
    ...
    public static final TransactionType ESCROW_RESULT = new AdvancedPayment() {
        ...
        @Override
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          throw new CaretException.NotValidException("Escrow result never validates");
        }
    ...
    public static final TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getFrequency() == null ||
             attachment.getFrequency() < Constants.CARET_SUBSCRIPTION_MIN_FREQ ||
             attachment.getFrequency() > Constants.CARET_SUBSCRIPTION_MAX_FREQ) {
            throw new CaretException.NotValidException("Invalid subscription frequency");
          }
          if (transaction.getAmountNQT() < Constants.ONE_CARET || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
            throw new CaretException.NotValidException("Subscriptions must be at least one caret");
          }
          if (transaction.getSenderId() == transaction.getRecipientId()) {
            throw new CaretException.NotValidException("Cannot create subscription to same address");
          }
          if (!subscriptionService.isEnabled()) {
            throw new CaretException.NotYetEnabledException("Subscriptions not yet enabled");
          }
        }
    ...
    public static final TransactionType SUBSCRIPTION_CANCEL = new AdvancedPayment() {
        ...
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          if (attachment.getSubscriptionId() == null) {
            throw new CaretException.NotValidException("Subscription cancel must include subscription id");
          }

          Subscription subscription = subscriptionService.getSubscription(attachment.getSubscriptionId());
          if (subscription == null) {
            throw new CaretException.NotValidException("Subscription cancel must contain current subscription id");
          }

          if (!subscription.getSenderId().equals(transaction.getSenderId()) &&
             !subscription.getRecipientId().equals(transaction.getSenderId())) {
            throw new CaretException.NotValidException("Subscription cancel can only be done by participants");
          }

          if (!subscriptionService.isEnabled()) {
            throw new CaretException.NotYetEnabledException("Subscription cancel not yet enabled");
          }
        }
    ...
    public static final TransactionType SUBSCRIPTION_PAYMENT = new AdvancedPayment() {
        ...
        @Override
        void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
          throw new CaretException.NotValidException("Subscription payment never validates");
        }
  ...
  public static abstract class AutomatedTransactions extends TransactionType{
    ...
    final void validateAttachment(Transaction transaction) throws CaretException.ValidationException {
      if (transaction.getAmountNQT() != 0) {
        throw new CaretException.NotValidException("Invalid automated transaction transaction");
      }
      doValidateAttachment(transaction);
    }
    ...
    public static final TransactionType AUTOMATED_TRANSACTION_CREATION = new AutomatedTransactions(){
        ...
        void doValidateAttachment(Transaction transaction)
          ...
          if (! fluxCapacitor.isActive(FeatureToggle.AUTOMATED_TRANSACTION_BLOCK, blockchain.getLastBlock().getHeight())) {
            throw new CaretException.NotYetEnabledException("Automated Transactions not yet enabled at height " + blockchain.getLastBlock().getHeight());
          }
          if (transaction.getSignature() != null && accountService.getAccount(transaction.getId()) != null) {
            ...
            if (existingAccount.getPublicKey() != null && !Arrays.equals(existingAccount.getPublicKey(), new byte[32]))
              throw new CaretException.NotValidException("Account with id already exists");
          }
          ...
          try {
            ...
            throw new CaretException.NotCurrentlyValidException("Invalid AT creation bytes", e);
          }
          ...
          if (transaction.getFeeNQT() <  requiredFee){
            throw new CaretException.NotValidException("Insufficient fee for AT creation. Minimum: " + Convert.toUnsignedLong(requiredFee / Constants.ONE_CARET));
          }
          if (fluxCapacitor.isActive(FeatureToggle.AT_FIX_BLOCK_3)) {
            if (attachment.getName().length() > Constants.MAX_AUTOMATED_TRANSACTION_NAME_LENGTH) {
              throw new CaretException.NotValidException("Name of automated transaction over size limit");
            }
            if (attachment.getDescription().length() > Constants.MAX_AUTOMATED_TRANSACTION_DESCRIPTION_LENGTH) {
              throw new CaretException.NotValidException("Description of automated transaction over size limit");
            }
          }
    ...
    public static final TransactionType AT_PAYMENT = new AutomatedTransactions() {
        ...
        void doValidateAttachment(Transaction transaction) throws CaretException.ValidationException {
          ...
          throw new CaretException.NotValidException("AT payment never validates");
        }
  ...
  protected Fee getBaselineFee(int height) {
    return new Fee((fluxCapacitor.isActive(PRE_DYMAXION, height) ? FEE_QUANT : ONE_CARET), 0);
  }
