...
public abstract class EntitySqlTable<T> extends DerivedSqlTable implements EntityTable<T> {
  ...
  protected EntitySqlTable(String table, TableImpl<?> tableClass, CaretKey.Factory<T> dbKeyFactory, DerivedTableManager derivedTableManager) {
    this(table, tableClass, dbKeyFactory, false, derivedTableManager);
  }

  EntitySqlTable(String table, TableImpl<?> tableClass, CaretKey.Factory<T> dbKeyFactory, boolean multiversion, DerivedTableManager derivedTableManager) {
  ...
  public final void checkAvailable(int height) {
    if (multiversion && height < Caret.getBlockchainProcessor().getMinRollbackHeight()) {
  ...
  @Override
  public T get(CaretKey nxtKey, int height) {
    DbKey dbKey = (DbKey) nxtKey;
  ...
  @Override
  public CaretIterator<T> getManyBy(Condition condition, int from, int to) {
    return getManyBy(condition, from, to, defaultSort());
  }

  @Override
  public CaretIterator<T> getManyBy(Condition condition, int from, int to, List<SortField> sort) {
  ...
  @Override
  public CaretIterator<T> getManyBy(Condition condition, int height, int from, int to) {
    return getManyBy(condition, height, from, to, defaultSort());
  }

  @Override
  public CaretIterator<T> getManyBy(Condition condition, int height, int from, int to, List<SortField> sort) {
    checkAvailable(height);
    DSLContext ctx = Db.getDSLContext();
    SelectQuery query = ctx.selectQuery();
    query.addFrom(tableClass);
  ...
  public CaretIterator<T> getManyBy(DSLContext ctx, SelectQuery query, boolean cache) {
    final boolean doCache = cache && Db.isInTransaction();
  ...
  @Override
  public CaretIterator<T> getAll(int from, int to) {
    return getAll(from, to, defaultSort());
  }

  @Override
  public CaretIterator<T> getAll(int from, int to, List<SortField> sort) {
    DSLContext ctx = Db.getDSLContext();
    SelectQuery query = ctx.selectQuery();
    query.addFrom(tableClass);
    if ( multiversion ) {
  ...
  @Override
  public CaretIterator<T> getAll(int height, int from, int to) {
    return getAll(height, from, to, defaultSort());
  }

  @Override
  public CaretIterator<T> getAll(int height, int from, int to, List<SortField> sort) {
    checkAvailable(height);
    DSLContext ctx = Db.getDSLContext();
    SelectQuery query = ctx.selectQuery();
    query.addFrom(tableClass);
