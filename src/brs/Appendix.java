...
public interface Appendix {
  ...
  abstract class AbstractAppendix implements Appendix {
    ...
    AbstractAppendix(int blockchainHeight) {
      this.version = (byte)(Caret.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchainHeight) ? 1 : 0);
    }
    ...
    public abstract void validate(Transaction transaction) throws CaretException.ValidationException;
  ...
  class Message extends AbstractAppendix {
    ...
    public Message(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
      ...
      if (messageLength > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
        throw new CaretException.NotValidException("Invalid arbitrary message length: " + messageLength);
      }
    ...
    @Override
    public void validate(Transaction transaction) throws CaretException.ValidationException {
      if (this.isText && transaction.getVersion() == 0) {
        throw new CaretException.NotValidException("Text messages not yet enabled");
      }
      if (transaction.getVersion() == 0 && transaction.getAttachment() != Attachment.ARBITRARY_MESSAGE) {
        throw new CaretException.NotValidException("Message attachments not enabled for version 0 transactions");
      }
      if (message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
        throw new CaretException.NotValidException("Invalid arbitrary message length: " + message.length);
      }
    }
  ...
  abstract class AbstractEncryptedMessage extends AbstractAppendix {
    ...
    private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws CaretException.NotValidException {
      super(buffer, transactionVersion);
    ...
    public void validate(Transaction transaction) throws CaretException.ValidationException {
      if (encryptedData.getData().length > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
        throw new CaretException.NotValidException("Max encrypted message length exceeded");
      }
      if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
        ...
        throw new CaretException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
      }
    }
  ...
  class EncryptedMessage extends AbstractEncryptedMessage {
    ...
    static EncryptedMessage parse(JSONObject attachmentData) throws CaretException.NotValidException {
    ...
    public EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws CaretException.ValidationException {
      super(buffer, transactionVersion);
    }
    ...
    public void validate(Transaction transaction) throws CaretException.ValidationException {
      ...
      if (! transaction.getType().hasRecipient()) {
        throw new CaretException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
      }
      if (transaction.getVersion() == 0) {
        throw new CaretException.NotValidException("Encrypted message attachments not enabled for version 0 transactions");
      }
    }
  ...
  class EncryptToSelfMessage extends AbstractEncryptedMessage {
    ...
    static EncryptToSelfMessage parse(JSONObject attachmentData) throws CaretException.NotValidException {
    ...
    public  EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws CaretException.ValidationException {
      super(buffer, transactionVersion);
    }
    ...
    public void validate(Transaction transaction) throws CaretException.ValidationException {
      super.validate(transaction);
      if (transaction.getVersion() == 0) {
        throw new CaretException.NotValidException("Encrypt-to-self message attachments not enabled for version 0 transactions");
      }
    }
  ...
  class PublicKeyAnnouncement extends AbstractAppendix {
    ...
    public void validate(Transaction transaction) throws CaretException.ValidationException {
      if (! transaction.getType().hasRecipient()) {
        throw new CaretException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
      }
      if (publicKey.length != 32) {
        throw new CaretException.NotValidException("Invalid recipient public key length: " + Convert.toHexString(publicKey));
      }
      long recipientId = transaction.getRecipientId();
      if (Account.getId(this.publicKey) != recipientId) {
        throw new CaretException.NotValidException("Announced public key does not match recipient accountId");
      }
      if (transaction.getVersion() == 0) {
        throw new CaretException.NotValidException("Public key announcements not enabled for version 0 transactions");
      }
      ...
      if (recipientAccount != null && recipientAccount.getPublicKey() != null && ! Arrays.equals(publicKey, recipientAccount.getPublicKey())) {
        throw new CaretException.NotCurrentlyValidException("A different public key for this account has already been announced");
      }
    }
