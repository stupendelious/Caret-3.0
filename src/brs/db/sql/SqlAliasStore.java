...
public class SqlAliasStore implements AliasStore {
  ...
  private static final DbKey.LongKeyFactory<Alias.Offer> offerDbKeyFactory = new DbKey.LongKeyFactory<Alias.Offer>("id") {
      @Override
      public CaretKey newKey(Alias.Offer offer) {
        return offer.dbKey;
      }
    };
  ...
  @Override
  public CaretKey.LongKeyFactory<Alias.Offer> getOfferDbKeyFactory() {
    return offerDbKeyFactory;
  }

  private static final CaretKey.LongKeyFactory<Alias> aliasDbKeyFactory = new DbKey.LongKeyFactory<Alias>("id") {

      @Override
      public CaretKey newKey(Alias alias) {
        return alias.dbKey;
      }
    };

  @Override
  public CaretKey.LongKeyFactory<Alias> getAliasDbKeyFactory() {
  ...
  protected void saveOffer(Alias.Offer offer) throws SQLException {
    try (DSLContext ctx = Db.getDSLContext()) {
      ctx.insertInto(
        ...
        offer.getId(), offer.getPriceNQT(), ( offer.getBuyerId() == 0 ? null : offer.getBuyerId() ), Caret.getBlockchain().getHeight()
      ).execute();
    }
  }
  ...
  protected void saveAlias(DSLContext ctx, Alias alias) {
    ctx.insertInto(ALIAS).
      ...
      set(ALIAS.HEIGHT, Caret.getBlockchain().getHeight()).execute();
  }

  private final VersionedEntityTable<Alias> aliasTable;

  @Override
  public CaretIterator<Alias> getAliasesByOwner(long accountId, int from, int to) {
