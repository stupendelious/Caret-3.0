...
public final class GetState extends APIServlet.APIRequestHandler {
  ...
  JSONStreamAware processRequest(HttpServletRequest req) {
    ...
    response.put("application", Caret.APPLICATION);
    response.put("version", Caret.VERSION);
    ...
    try (CaretIterator<Account> accounts = accountService.getAllAccounts(0, -1)) {
      while(accounts.hasNext()) {
        long effectiveBalanceCARET = accounts.next().getBalanceNQT();
        if (effectiveBalanceCARET > 0) {
          totalEffectiveBalance += effectiveBalanceCARET;
        }
      }
    }
    try(CaretIterator<Escrow> escrows = escrowService.getAllEscrowTransactions()) {
      while(escrows.hasNext()) {
        totalEffectiveBalance += escrows.next().getAmountNQT();
      }
    }
    response.put("totalEffectiveBalanceNXT", totalEffectiveBalance / Constants.ONE_CARET);
    ...
    Peer lastBlockchainFeeder = Caret.getBlockchainProcessor().getLastBlockchainFeeder();
    response.put("lastBlockchainFeeder", lastBlockchainFeeder == null ? null : lastBlockchainFeeder.getAnnouncedAddress());
    response.put("lastBlockchainFeederHeight", Caret.getBlockchainProcessor().getLastBlockchainFeederHeight());
    response.put("isScanning", Caret.getBlockchainProcessor().isScanning());
