...
case "$MY_CMD" in
    "sonarcube")
        ...
        ;;
    "debug")
        mvn exec:exec -Dexec.executable=java "-Dexec.args=-classpath %classpath:conf -agentlib:jdwp=transport=dt_socket,server=n,address=127.0.0.1:8000,suspend=y -Dgreeting=\"Hello\" -Ddev=true brs.Caret"
        ;;
    "test")
	mvn -Dtest=$2 test
        ;;
