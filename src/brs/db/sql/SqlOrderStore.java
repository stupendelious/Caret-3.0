...
public class SqlOrderStore implements OrderStore {
  protected DbKey.LongKeyFactory<Order.Ask> askOrderDbKeyFactory = new DbKey.LongKeyFactory<Order.Ask>("id") {
      ...
      @Override
      public CaretKey newKey(Order.Ask ask) {
        return ask.dbKey;
      }
  ...
  private DbKey.LongKeyFactory<Order.Bid> bidOrderDbKeyFactory = new DbKey.LongKeyFactory<Order.Bid>("id") {
      ...
      @Override
      public CaretKey newKey(Order.Bid bid) {
        return bid.dbKey;
      }
  ...
  @Override
  public CaretIterator<Order.Ask> getAskOrdersByAccountAsset(final long accountId, final long assetId, int from, int to) {
  ...
  @Override
  public CaretIterator<Order.Ask> getSortedAsks(long assetId, int from, int to) {
    List<SortField> sort = new ArrayList<>();
  ...
  public Order.Ask getNextOrder(long assetId) {
    ...
    try (CaretIterator<Order.Ask> askOrders = askOrderTable.getManyBy(ctx, query, true)) {
      return askOrders.hasNext() ? askOrders.next() : null;
    }
  }

  @Override
  public CaretIterator<Order.Ask> getAll(int from, int to) {
    return askOrderTable.getAll(from, to);
  }

  @Override
  public CaretIterator<Order.Ask> getAskOrdersByAccount(long accountId, int from, int to) {
    return askOrderTable.getManyBy(brs.schema.Tables.ASK_ORDER.ACCOUNT_ID.eq(accountId), from, to);
  }

  @Override
  public CaretIterator<Order.Ask> getAskOrdersByAsset(long assetId, int from, int to) {
  ...
  private void saveAsk(DSLContext ctx, TableImpl table, Order.Ask ask) throws SQLException {
    ...
    askOrderRecord.setHeight(brs.Caret.getBlockchain().getHeight());
  ...
  @Override
  public CaretIterator<Order.Bid> getBidOrdersByAccount(long accountId, int from, int to) {
    return bidOrderTable.getManyBy(brs.schema.Tables.BID_ORDER.ACCOUNT_ID.eq(accountId), from, to);
  }

  @Override
  public CaretIterator<Order.Bid> getBidOrdersByAsset(long assetId, int from, int to) {
    return bidOrderTable.getManyBy(brs.schema.Tables.BID_ORDER.ASSET_ID.eq(assetId), from, to);
  }

  @Override
  public CaretIterator<Order.Bid> getBidOrdersByAccountAsset(final long accountId, final long assetId, int from, int to) {
  ...
  @Override
  public CaretIterator<Order.Bid> getSortedBids(long assetId, int from, int to) {
    List<SortField> sort = new ArrayList<>();
  ...
  public Order.Bid getNextBid(long assetId) {
    ...
    try (CaretIterator<Order.Bid> bidOrders = bidOrderTable.getManyBy(ctx, query, true)) {
  ...
  private void saveBid(DSLContext ctx, TableImpl table, Order.Bid bid) throws SQLException {
    ...
    bidOrderRecord.setHeight(brs.Caret.getBlockchain().getHeight());
