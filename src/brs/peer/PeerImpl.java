...
final class PeerImpl implements Peer {
  ...
  public boolean isAtLeastMyVersion() {
    return isHigherOrEqualVersionThan(Caret.VERSION.substring(0, Caret.VERSION.lastIndexOf(".")) + ".0");
  }
  
  void setVersion(String version) {
    this.version = version;
    isOldVersion = false;
    if (Caret.APPLICATION.equals(application) && version != null) {
  ...
  public void blacklist(Exception cause, String description) {
    if (cause instanceof CaretException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
...
*/
  ...
  public JSONObject send(final JSONStreamAware request) {
    ...
    try {
      ...
      if (port <= 0) {
        buf.append(':');
        buf.append(Caret.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Peers.TESTNET_PEER_PORT : Peers.DEFAULT_PEER_PORT);
      }
      buf.append("/caret");
      ...
      connection.addRequestProperty("User-Agent","BRS/" + Caret.VERSION);
