...
public class SqlDigitalGoodsStoreStore implements DigitalGoodsStoreStore {
  ...
  private static final DbKey.LongKeyFactory<DigitalGoodsStore.Purchase> feedbackDbKeyFactory
    = new DbKey.LongKeyFactory<DigitalGoodsStore.Purchase>("id") {
        @Override
        public CaretKey newKey(DigitalGoodsStore.Purchase purchase) {
          return purchase.dbKey;
        }
      };

  protected final CaretKey.LongKeyFactory<DigitalGoodsStore.Purchase> purchaseDbKeyFactory
    = new DbKey.LongKeyFactory<DigitalGoodsStore.Purchase>("id") {
        @Override
        public CaretKey newKey(DigitalGoodsStore.Purchase purchase) {
          return purchase.dbKey;
  ...
  private final CaretKey.LongKeyFactory<DigitalGoodsStore.Goods> goodsDbKeyFactory = new DbKey.LongKeyFactory<DigitalGoodsStore.Goods>("id") {
      @Override
      public CaretKey newKey(DigitalGoodsStore.Goods goods) {
        return goods.dbKey;
      }
    };
  ...
  public SqlDigitalGoodsStoreStore(DerivedTableManager derivedTableManager) {
    ...
    feedbackTable = new VersionedValuesSqlTable<DigitalGoodsStore.Purchase, EncryptedData>("purchase_feedback", brs.schema.Tables.PURCHASE_FEEDBACK, feedbackDbKeyFactory, derivedTableManager) {
      ...
      protected void save(DSLContext ctx, DigitalGoodsStore.Purchase purchase, EncryptedData encryptedData) {
        ...
        ctx.insertInto(
            ...
            purchase.getId(),
            data, nonce,
            brs.Caret.getBlockchain().getHeight(), true
        ).execute();
      }
    };
    ...
    publicFeedbackTable
      ...
      protected void save(DSLContext ctx, DigitalGoodsStore.Purchase purchase, String publicFeedback) throws SQLException {
        ...
        feedbackRecord.setHeight(brs.Caret.getBlockchain().getHeight());
  ...
  @Override
  public CaretIterator<DigitalGoodsStore.Purchase> getExpiredPendingPurchases(final int timestamp) {
  ...
  @Override
  public CaretKey.LongKeyFactory<DigitalGoodsStore.Purchase> getFeedbackDbKeyFactory() {
    return feedbackDbKeyFactory;
  }

  @Override
  public CaretKey.LongKeyFactory<DigitalGoodsStore.Purchase> getPurchaseDbKeyFactory() {
  ...
  @Override
  public CaretKey.LongKeyFactory<DigitalGoodsStore.Goods> getGoodsDbKeyFactory() {
  ...
  protected void saveGoods(DSLContext ctx, DigitalGoodsStore.Goods goods) throws SQLException {
    ...
    goodsRecord.setHeight(brs.Caret.getBlockchain().getHeight());
  ...
  protected void savePurchase(DSLContext ctx, DigitalGoodsStore.Purchase purchase) throws SQLException {
    ...
    purchaseRecord.setHeight(Caret.getBlockchain().getHeight());
  ...
  @Override
  public CaretIterator<DigitalGoodsStore.Goods> getGoodsInStock(int from, int to) {
    return goodsTable.getManyBy(GOODS.DELISTED.isFalse().and(GOODS.QUANTITY.gt(0)), from, to);
  }

  @Override
  public CaretIterator<DigitalGoodsStore.Goods> getSellerGoods(final long sellerId, final boolean inStockOnly, int from, int to) {
    List<SortField> sort = new ArrayList<>();
  ...
  @Override
  public CaretIterator<DigitalGoodsStore.Purchase> getAllPurchases(int from, int to) {
    return purchaseTable.getAll(from, to);
  }

  @Override
  public CaretIterator<DigitalGoodsStore.Purchase> getSellerPurchases(long sellerId, int from, int to) {
    return purchaseTable.getManyBy(PURCHASE.SELLER_ID.eq(sellerId), from, to);
  }

  @Override
  public CaretIterator<DigitalGoodsStore.Purchase> getBuyerPurchases(long buyerId, int from, int to) {
    return purchaseTable.getManyBy(PURCHASE.BUYER_ID.eq(buyerId), from, to);
  }

  @Override
  public CaretIterator<DigitalGoodsStore.Purchase> getSellerBuyerPurchases(final long sellerId, final long buyerId, int from, int to) {
    return purchaseTable.getManyBy(PURCHASE.SELLER_ID.eq(sellerId).and(PURCHASE.BUYER_ID.eq(buyerId)), from, to);
  }

  @Override
  public CaretIterator<DigitalGoodsStore.Purchase> getPendingSellerPurchases(final long sellerId, int from, int to) {
