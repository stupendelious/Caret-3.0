...
public class SqlBlockchainStore implements BlockchainStore {
  ...
  private final TransactionDb transactionDb = Caret.getDbs().getTransactionDb();
  private final BlockDb blockDb = Caret.getDbs().getBlockDb();

  @Override
  public CaretIterator<Block> getBlocks(int from, int to) {
    try ( DSLContext ctx = Db.getDSLContext() ) {
      int blockchainHeight = Caret.getBlockchain().getHeight();
      return
        getBlocks(
          ctx,
  ...
  @Override
  public CaretIterator<Block> getBlocks(Account account, int timestamp, int from, int to) {
    try ( DSLContext ctx = Db.getDSLContext() ) {
      int blockchainHeight      = Caret.getBlockchain().getHeight();
  ...
  @Override
  public CaretIterator<Block> getBlocks(DSLContext ctx, ResultSet rs) {
  ...
  @Override
  public CaretIterator<Transaction> getAllTransactions() {
    DSLContext ctx = Db.getDSLContext();
  ...
  @Override
  public CaretIterator<Transaction> getTransactions(Account account, int numberOfConfirmations, byte type, byte subtype,
                                                        int blockTimestamp, int from, int to) {
    int height = numberOfConfirmations > 0 ? Caret.getBlockchain().getHeight() - numberOfConfirmations : Integer.MAX_VALUE;
    if (height < 0) {
      throw new IllegalArgumentException("Number of confirmations required " + numberOfConfirmations
                                         + " exceeds current blockchain height " + Caret.getBlockchain().getHeight());
    }
    DSLContext ctx = Db.getDSLContext();
  ...
  @Override
  public CaretIterator<Transaction> getTransactions(DSLContext ctx, ResultSet rs) {
  ...
  @Override
  public CaretIterator<Block> getLatestBlocks(int amountBlocks) {
