...
public class DGSGoodsStoreServiceImpl implements DGSGoodsStoreService {
  ...
  @Override
  public CaretIterator<Goods> getAllGoods(int from, int to) {
    return goodsTable.getAll(from, to);
  }

  @Override
  public CaretIterator<Goods> getGoodsInStock(int from, int to) {
    return digitalGoodsStoreStore.getGoodsInStock(from, to);
  }

  @Override
  public CaretIterator<Goods> getSellerGoods(final long sellerId, final boolean inStockOnly, int from, int to) {
    return digitalGoodsStoreStore.getSellerGoods(sellerId, inStockOnly, from, to);
  }

  @Override
  public CaretIterator<Purchase> getAllPurchases(int from, int to) {
    return purchaseTable.getAll(from, to);
  }

  @Override
  public CaretIterator<Purchase> getSellerPurchases(long sellerId, int from, int to) {
    return digitalGoodsStoreStore.getSellerPurchases(sellerId, from, to);
  }

  @Override
  public CaretIterator<Purchase> getBuyerPurchases(long buyerId, int from, int to) {
    return digitalGoodsStoreStore.getBuyerPurchases(buyerId, from, to);
  }

  @Override
  public CaretIterator<Purchase> getSellerBuyerPurchases(final long sellerId, final long buyerId, int from, int to) {
    return digitalGoodsStoreStore.getSellerBuyerPurchases(sellerId, buyerId, from, to);
  }

  @Override
  public CaretIterator<Purchase> getPendingSellerPurchases(final long sellerId, int from, int to) {
  ...
  public void listGoods(Transaction transaction, Attachment.DigitalGoodsListing attachment) {
    CaretKey dbKey = goodsDbKeyFactory.newKey(transaction.getId());
  ...
  @Override
  public CaretIterator<Purchase> getExpiredPendingPurchases(final int timestamp) {
