...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.login = function(password, callback) {
        if (!password.length) {
            ...
            if ( BRS.hasLocalStorage ) {
                if ( BRS.settings.remember_passphrase ) {
                    localStorage.setItem("caret.passphrase", $("#login_password").val());
                }
                else {
                    localStorage.removeItem("caret.passphrase");
                }
            }
        }
        ...
        BRS.sendRequest("getBlockchainStatus", function(response) {
            ...
            var login_response_function = function(response) {
                ...
                BRS.sendRequest("getAccountPublicKey", {
                    ...
                    BRS.getAccountInfo(true, function() {
                        ...
                        if (!BRS.accountInfo.publicKey || BRS.accountInfo.effectiveBalanceCARET === 0 || !BRS.isLocalHost || BRS.downloadingBlockchain || BRS.isLeased) {
            ...
            if (password.trim().toUpperCase().substring(0, 6) === "CARET-" && password.length === 26) {
