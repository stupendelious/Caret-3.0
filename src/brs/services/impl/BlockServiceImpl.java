...
public class BlockServiceImpl implements BlockService {
  ...
  public boolean verifyBlockSignature(Block block) throws BlockchainProcessor.BlockOutOfOrderException {
    try {
      ...
      if (genAccount == null || rewardAssignment == null || previousBlock.getHeight()
          + 1 < Constants.CARET_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK) {
  ...
  public void apply(Block block) {
    ...
    if (block.getHeight() < Constants.CARET_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK) {
  ...
  public long getBlockReward(Block block) {
    if (block.getHeight() == 0 || block.getHeight() >= 5832000) {
      return 0;
    }
    int month = block.getHeight() / 32400;
    return BigInteger.valueOf(1000).multiply(BigInteger.valueOf(95).pow(month))
        .divide(BigInteger.valueOf(100).pow(month)).longValue() * Constants.ONE_CARET;
  }
  ...
  public void calculateBaseTarget(Block block, Block previousBlock) throws BlockOutOfOrderException {
    ...
    } else if (block.getHeight() < Constants.CARET_DIFF_ADJUST_CHANGE_BLOCK) {
