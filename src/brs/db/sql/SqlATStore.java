...
public class SqlATStore implements ATStore {
  ...
  private final CaretKey.LongKeyFactory<brs.AT> atDbKeyFactory = new DbKey.LongKeyFactory<brs.AT>("id") {
      @Override
      public CaretKey newKey(brs.AT at) {
        return at.dbKey;
      }
    };
  private final VersionedEntityTable<brs.AT> atTable;

  private final CaretKey.LongKeyFactory<brs.AT.ATState> atStateDbKeyFactory = new DbKey.LongKeyFactory<brs.AT.ATState>("at_id") {
      @Override
      public CaretKey newKey(brs.AT.ATState atState) {
  ...
  protected void saveATState(DSLContext ctx, brs.AT.ATState atState) throws SQLException {
    ...
    atStateRecord.setHeight(Caret.getBlockchain().getHeight());
  ...
  protected void saveAT(DSLContext ctx, brs.AT at) {
    ctx.insertInto(
      ...
      brs.AT.compressState(at.getApCode()), Caret.getBlockchain().getHeight()
    ).execute();
  }
  ...
  public List<Long> getOrderedATs() {
    ...
    return ctx.selectFrom(
      ...
      AT_STATE.NEXT_HEIGHT.lessOrEqual( Caret.getBlockchain().getHeight() + 1)
    ).and(
      ACCOUNT.BALANCE.greaterOrEqual(
        AT_Constants.getInstance().STEP_FEE(Caret.getBlockchain().getHeight())
        * AT_Constants.getInstance().API_STEP_MULTIPLIER(Caret.getBlockchain().getHeight())
      )
    ).and(
  ...
  @Override
  public CaretKey.LongKeyFactory<brs.AT> getAtDbKeyFactory() {
  ...
  @Override
  public CaretKey.LongKeyFactory<brs.AT.ATState> getAtStateDbKeyFactory() {
